...
# version-1.2.4 - 2015-01-23 at 19:09 EST
...
# Optional: full (dated or plain) backup of raw Ulysses Libraries, using rsync.
do_icloud_backup = False
do_onmymac_backup = False

import os
import datetime
HOME = os.getenv("HOME", "") + "/"
time_stamp = "_" + datetime.datetime.now().strftime("%Y-%m-%d_%H%M")
# date_stamp = "_" + datetime.datetime.now().strftime("%Y-%m-%d")
...
# Paths for Ulysses Backups:
backup_path_icloud = HOME + "Backup/Ulysses_iCloud_Backup" + time_stamp
backup_path_mac = HOME + "Backup/Ulysses_OnMyMac_Backup" + time_stamp
...
def main():
    ...
    if do_icloud_backup:
        if not os.path.exists(backup_path_icloud):
            os.makedirs(backup_path_icloud)
        print("*** Backing up iCloud library to:", backup_path_icloud)
        subprocess.call(['rsync', '-t', '-r', '--delete',
                        ulysses_path_icloud, backup_path_icloud])
        print("*** end iCloud backup")
        print()

    if do_onmymac_backup:
        if not os.path.exists(backup_path_mac):
            os.makedirs(backup_path_mac)
        print("*** Backing up OnMyMac library to:", backup_path_mac)
        subprocess.call(['rsync', '-t', '-r', '--delete',
                        ulysses_path_mac, backup_path_mac])
        print("*** end OnMyMac backup")
        print()

    ulysses_path_lib = ulysses_path_icloud + "Documents/Library/"
    # ulysses_path_lib = ulysses_path_mac + "Documents/Library/"
